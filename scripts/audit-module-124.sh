#!/bin/bash
# Audit module #124 for unused code and duplicated helpers
...
echo -e "${BLUE}═══════════════════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  Module #124 Audit - Unused Code & Duplicated Helpers${NC}"
echo -e "${BLUE}═══════════════════════════════════════════════════════════════${NC}"
echo ""

# Find the module
MODULE_PATH="./packages/module-124"
if [ ! -d "$MODULE_PATH" ]; then
    MODULE_PATH="./src/modules/124"
fi
if [ ! -d "$MODULE_PATH" ]; then
    MODULE_PATH="./modules/124"
fi
if [ ! -d "$MODULE_PATH" ]; then
    echo -e "${RED}❌ Module #124 not found${NC}"
    echo "Searching for module 124..."
    find . -type d -name "*124*" 2>/dev/null
    exit 1
fi
...
# 5. Find unused functions
echo -e "${YELLOW}📋 Step 5: Checking for unused functions...${NC}"
find "$MODULE_PATH" -name "*.ts" -o -name "*.tsx" | while read -r file; do
    grep -E "function [a-zA-Z_][a-zA-Z0-9_]*" "$file" 2>/dev/null | while read -r line; do
        func_name=$(echo "$line" | sed -E 's/function ([a-zA-Z_][a-zA-Z0-9_]*).*/\1/')
        if [ -n "$func_name" ]; then
            usage_count=$(grep -v "^[[:space:]]*function" "$file" | grep -c "$func_name" 2>/dev/null || echo 0)
            if [ "$usage_count" -eq 0 ]; then
                echo -e "  ${YELLOW}⚠️ Possibly unused function: $func_name in $file${NC}"
            fi
        fi
    done
done
...
# 6. Verify tests
echo -e "${YELLOW}📋 Step 6: Verifying tests...${NC}"
...
if [ -n "$TEST_PATH" ]; then
    ...
    echo -e "  ${YELLOW}⚠️ No tests found for module #124${NC}"
fi
echo ""
