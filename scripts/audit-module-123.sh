#!/bin/bash
# Audit module #123 for unused code and duplicated helpers
...
echo -e "${BLUE}═══════════════════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  Module #123 Audit - Unused Code & Duplicated Helpers${NC}"
echo -e "${BLUE}═══════════════════════════════════════════════════════════════${NC}"
echo ""

# Find the module
MODULE_PATH="./packages/module-123"
if [ ! -d "$MODULE_PATH" ]; then
    MODULE_PATH="./src/modules/123"
fi
if [ ! -d "$MODULE_PATH" ]; then
    MODULE_PATH="./modules/123"
fi
if [ ! -d "$MODULE_PATH" ]; then
    echo -e "${RED}❌ Module #123 not found${NC}"
    echo "Searching for module 123..."
    find . -type d -name "*123*" 2>/dev/null
    exit 1
fi
...
if [ -n "$TEST_PATH" ]; then
    ...
    echo -e "  ${YELLOW}⚠️ No tests found for module #123${NC}"
fi
echo ""
